"""Tests for Amazon Kindle multi-stakeholder pipeline.

Tests stakeholder utility definitions, preference pair generation,
BT training convergence on 32-dim features, stakeholder differentiation,
and the orthogonal-by-construction premium_seller pairs.
"""
...
_amazon_mod = _load_module("amazon", _root / "enhancements" / "data" / "amazon.py")
_stakeholders_mod = _load_module(
    "amazon_stakeholders", _root / "enhancements" / "data" / "amazon_stakeholders.py"
)

AmazonDataset = _amazon_mod.AmazonDataset
...
generate_amazon_content_pool = _stakeholders_mod.generate_amazon_content_pool
generate_amazon_preferences = _stakeholders_mod.generate_amazon_preferences

DATA_DIR = Path("data/amazon-kindle")
STAKEHOLDER_NAMES = ["reader", "publisher", "indie_author", "premium_seller", "diversity"]
ORTHOGONAL_PAIRS = [
    ("publisher", "premium_seller"),
    ("indie_author", "premium_seller"),
    ("premium_seller", "diversity"),
]


@pytest.fixture(scope="module")
def dataset() -> AmazonDataset:
    if not DATA_DIR.exists():
        pytest.skip("Amazon Kindle data not available at data/amazon-kindle")
    return AmazonDataset(str(DATA_DIR))


@pytest.fixture(scope="module")
def stakeholder_weights(dataset: AmazonDataset) -> dict[str, np.ndarray]:
    return build_stakeholder_configs(dataset)


@pytest.fixture(scope="module")
def content_pool(dataset: AmazonDataset) -> tuple[np.ndarray, np.ndarray]:
    return generate_amazon_content_pool(dataset, min_ratings=5, seed=42)
    ...
    def test_weights_normalized(self, stakeholder_weights: dict[str, np.ndarray]) -> None:
        for name, weights in stakeholder_weights.items():
            assert np.max(np.abs(weights)) == pytest.approx(1.0, abs=0.01), (
                f"{name} weights not unit-max-normalized"
            )

    def test_orthogonal_pairs(self, stakeholder_weights: dict[str, np.ndarray]) -> None:
        """Premium_seller uses disjoint feature slots, producing exact-zero cosines."""
        for a, b in ORTHOGONAL_PAIRS:
            wa, wb = stakeholder_weights[a], stakeholder_weights[b]
            cos = float(np.dot(wa, wb) / (np.linalg.norm(wa) * np.linalg.norm(wb) + 1e-12))
            assert abs(cos) < 1e-9, (
                f"{a}-{b} raw cosine {cos:.6f} should be exactly 0"
            )
...
class TestContentPool:
    ...
    def test_content_pool_shape(self, content_pool: tuple[np.ndarray, np.ndarray]) -> None:
        ...
        assert features.shape[0] > 5000, f"Pool too small: {features.shape[0]}"
    ...
    def test_topics_valid(self, content_pool: tuple[np.ndarray, np.ndarray]) -> None:
        ...
        assert np.all(topics < 20), "Topics should be in [0, 19] for 20 Amazon genres"
...
class TestPreferenceGeneration:
    ...
    def test_preference_shape(
        ...
        features, _ = content_pool
        pref, rej = generate_amazon_preferences(
    ...
    def test_preference_deterministic(
        ...
        features, _ = content_pool
        pref1, rej1 = generate_amazon_preferences(
            features, stakeholder_weights["reader"], n_pairs=50, seed=123
        )
        pref2, rej2 = generate_amazon_preferences(
            features, stakeholder_weights["reader"], n_pairs=50, seed=123
        )
    ...
    def test_different_seeds_differ(
        ...
        features, _ = content_pool
        pref1, _ = generate_amazon_preferences(
            features, stakeholder_weights["reader"], n_pairs=50, seed=1
        )
        pref2, _ = generate_amazon_preferences(
...
class TestStakeholderDisagreement:
    ...
    def test_minimum_disagreement(
        ...
        for a, b in combinations(STAKEHOLDER_NAMES, 2):
        ...
        assert high_disagreement_count >= 5, (
            f"Only {high_disagreement_count} pairs have >5% disagreement"
        )


class TestBTConvergence:
    """Test that BT training converges on D=32 Amazon features."""
    ...
    def test_stakeholder_differentiation(
        ...
        features, _ = content_pool
        trained = {}
        for name in ["reader", "publisher", "diversity"]:
            pref, rej = generate_amazon_preferences(
