# F2 Retrospective: Reward Modeling

**Feature**: F2 — Reward Modeling for Multi-Stakeholder Recommendations
...
Source: `results/f2_phase2_two_stage/two_stage_comparison.json` — Topic-aware avg_purity: 1.0, Topic×Action avg_purity: 1.0. The lesson: feature engineering > model complexity. Using per-topic engagement features solved clustering perfectly, making complex end-to-end models unnecessary.
...
Source: `results/f2_phase3_causal/stress_test_results.json`, `results/f2_phase3_causal/causal_verification_results.json`.
...
This is arguably F2's most important lesson and corrects the Phase 2 narrative that attributed weight convergence to "fundamental BT limitations." BT's scale invariance is real (you can't recover exact weight *values*), but weight *differentiation* across groups was always solvable — it just required different preference labels per group.

### "Weight recovery matters"

Phase 2 invested heavily in recovering ground truth weights (target: 0.8 correlation). Best result: 0.554 (`results/f2_phase2_two_stage/two_stage_summary.json`, mean_correlation). This was declared a fundamental limitation and accepted.
...
F2's original question — "given stakeholder utility definitions, can we train differentiated models?" — is answered. The natural next questions are about identifiability and robustness: what can and can't be recovered from preference data, and how sensitive are the results to the choices we made?
...
- **Disagreement → differentiation bound** *(resolved)*: Swept neg-penalty α from 0.1 to 8.0 across 34 training runs (two sweeps: Platform-fixed and User-fixed). Findings: (1) the relationship is strongly monotonic (Spearman ρ = -0.984), (2) best combined fit is linear with R² = 0.898, (3) thresholds: ≥10% disagreement for cos < 0.95, ≥19% for cos < 0.80, ≥37% for cos < 0.50. The initial fit was reference-dependent — the same disagreement rate produced different cosine similarities depending on which stakeholder was the reference. Adding mean total margin (average utility gap on disagreed pairs) as a second predictor resolves this: the 2-variable model `cos = 1.098 − 1.127d − 0.088m` achieves R² = 0.977. For the utility family U = pos − α·neg, mean margin decomposes as |Δα| · E[Δneg | disagreement], cleanly separating parameter distance from content structure. **Conclusion**: disagreement rate + mean margin together give a near-deterministic predictor of weight differentiation. See `results/disagreement_bound_analysis.json`.
- **LLM confidence as margin proxy** *(resolved — CONDITIONAL_GO)*: Tested whether the 2-variable structure (disagreement rate + depth signal) is robust when the annotator uses a *different* utility function. Gave Claude Haiku natural language stakeholder descriptions ("you're safety-first" vs "you're engagement-focused") and simplified content scores (engagement 0-100, negativity 0-100) — no formula. The LLM applied its own implicit utility function. Results: LLM disagreement rate strongly tracks analytic disagreement (Pearson = 0.921), and the LLM 2-var model (d_llm + confidence) ranks cosine similarities correctly (Spearman = 0.929), though with lower linear fit (R² = 0.80) due to confidence compression. **Conclusion**: the 2-variable prediction structure is robust across utility functions — a practitioner's LLM-annotated preferences carry enough signal to rank stakeholder differentiation, even when their implicit utility differs from the parametric one. LLM confidence adds ranking value (+0.135 Spearman boost over d alone) but is too compressed in range [1.40, 1.66] to serve as a precise margin substitute. **Goalpost change**: the original MUST criterion was R² ≥ 0.85 for the LLM 2-var model. We changed this to Spearman ≥ 0.85 during analysis, recognizing that rank-ordering is the practically relevant question — if the model correctly ranks which stakeholder pairs are most differentiated, a practitioner can always fit a calibration curve. R² penalizes nonlinear-but-monotone relationships that are equally useful. See `results/llm_margin_proxy.json`.
- **α-recovery from weight vectors** *(resolved)*: For the utility family U = pos − α·neg, trained BT models across 13 α values (0.1–8.0) and tested whether α is recoverable from the learned weight vector via the ratio -mean(w_neg)/mean(w_pos). **Result: perfect monotonic recovery** — Spearman = 1.0000 across all 13 points. Pearson = 0.9985, R² = 0.9971. The recovered α follows an affine transform: α_rec ≈ -0.062 + 1.321 · α_true (systematic amplification, slope 1.32 instead of 1.0). Stakeholder validation: platform (0.3 → 0.344), user (1.0 → 1.345), society (4.0 → 5.594). All three recovery methods (mean ratio, sum ratio, regression) achieve identical Spearman. **Conclusion**: BT's scale invariance prevents recovering absolute weight magnitudes, but the *ratio* structure within the weight vector perfectly encodes the utility parameter α up to an affine transform. This closes out the "relative stakeholder importance" question — it IS recoverable. See `results/alpha_recovery.json`.
- **α-recovery stress tests** *(resolved)*: The baseline recovery (Spearman = 1.0) used ideal conditions. Stress-tested across 4 dimensions (1300 training runs, 5 seeds each, 13 α values). Breaking points (Spearman drops below 0.95): label noise p_flip=0.30 (robust through 20%), sample size n=50 pairs (250 pairs suffices), BT temperature β=0.5 (strongest stressor — β=0.2 collapses to 0.61), content correlation ρ=0.8 (but ρ=0.3 *improves* to perfect 1.0). Key finding: Spearman (rank ordering) degrades much slower than Pearson (linear fit) — the ordering of α values is robust to conditions that destroy the linear relationship. **Conclusion**: α-recovery is practically reliable with ≥250 pairs, ≤20% label noise, and preference temperature β ≥ 0.5. Content correlation helps up to moderate levels. See `results/alpha_recovery_stress.json`.
- **What's recoverable vs not** *(resolved)*: Weight magnitudes (no — BT scale invariance), weight ordering (yes — Spearman ρ = 0.767), relative stakeholder importance via α (yes — Spearman ≥ 0.95 with ≥250 pairs, ≤20% noise, β ≥ 0.5). Direction 1 is now fully resolved.
...
All results above assume linear utility (U = pos − α·neg). We tested whether F2's claims hold under two nonlinear departures: **Concave** (U = pos^γ − α·neg^γ, prospect theory diminishing returns) and **Threshold** (U = pos − α·max(neg−τ, 0), ignore low negativity). Three experiments, 156+ training runs:
...
All three directions share F2's infrastructure and build on each other:
...
| `scripts/` (F2-related) | 15+ | Training, evaluation, analysis, comparison scripts |

### Key quantitative results

| Metric | Source File | Value |
|--------|------------|-------|
| Phase 1 val accuracy | `results/f2_phase1/training_metrics.json` | 99.3% |
| Phase 1 standard accuracy | `results/f2_phase1/comprehensive_evaluation.json` | 100% |
| Phase 1 label flip impact (30%) | `docs/results.md:1383` | 68.7% accuracy |
| Phase 2 accuracy (topic-aware) | `results/f2_phase2_two_stage/two_stage_comparison.json` | 99.3% |
| Phase 2 cluster purity (topic-aware) | same | 100% (avg_purity: 1.0) |
| Phase 2 weight correlation | `results/f2_phase2_two_stage/two_stage_summary.json` | 0.554 |
| Phase 3 block/follow pass rate | `results/f2_phase3_causal/causal_verification_results.json` | 100% |
| Phase 3 history pass rate | same | 50% |
| Phase 3 stress tests | `results/f2_phase3_causal/stress_test_results.json` | 7/7 |
...
F2 produces 18-dimensional weight vectors stored in JSON files. F1 produces an optimized inference pipeline with JIT, KV-cache, and quantization. There is no system that combines them into a serving-time recommendation pipeline. No A/B testing framework. No real-data ingestion. No monitoring or feedback loop.

The entire F2 body of work is a research prototype with rigorous internal validation but zero production readiness.
...
Deeper analysis of the F2 journey, organized thematically.
...
F2's contribution: the learned 18D BT weight vector replaces the 3-coefficient scoring function while reusing the same mechanism. Result: user_utility 1.149 vs 1.087 hardcoded — a 5.7% improvement from better scoring alone (`results/pareto_comparison.json`).
...
### Was utility function design the original F2 question?

No. The design doc (`docs/design_doc.md:365-448`) framed F2 as three tiers:
...
F2 answered that question: yes, with stakeholder-specific preference labels. The deeper question — *how to design the right utility functions for real data* — emerges from F2 as the natural next-order problem. It's not a gap in F2; it's the question F2 unlocks.
...
F2 is a genuine research contribution: it discovered (and rigorously proved through 87 experiments) that Bradley-Terry preference learning differentiates stakeholders through training labels, not loss functions. It built a complete verification stack from synthetic ground truth through causal intervention tests. It validated on MovieLens (real data) and a 648-parameter synthetic Twitter ground truth.
