...
The direction condition is validated on 6 data points across 3 datasets
(synthetic, ML-100K, ML-1M), with zero violations:
...
\midrule
Synthetic & platform & $+0.893$ & Improve & improving \checkmark \\
Synthetic & society & $+0.843$ & Improve & improving \checkmark \\
...
\textbf{Direction condition holds perfectly: 6/6.} Every stakeholder
with $\cos < 0$ degrades with $N$; every stakeholder with $\cos > 0$
improves.

\textbf{Key finding on synthetic:} The synthetic benchmark has
\emph{no Goodhart} under the utility-based metric, because all
stakeholder pairs have $\cos > 0$ (user-society $= +0.84$,
user-platform $= +0.89$). Society utility \emph{improves} from 3.21
($N=25$) to 5.34 ($N=2000$). The original paper's Hausdorff-based
``Goodhart'' on synthetic was measuring frontier distance (which
increases with optimization precision), not hidden stakeholder
degradation. The direction condition correctly predicts: no
anti-correlation $\Rightarrow$ no Goodhart.
...
\end{itemize}
Validated on 6 data points across 3 datasets (synthetic, ML-100K, ML-1M)
with 0 violations.
\end{proposition}
