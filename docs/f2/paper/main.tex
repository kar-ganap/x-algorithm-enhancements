...
foregrounding societal welfare as a first-class stakeholder objective.
...
that must be traded off on a frontier, and the critical question is
...
stakeholder's utility may be independently misspecified.
...
Iancu and Trichakis~\cite{iancu2014pareto} extend this to
...
\begin{itemize}
  ...
  \item \textbf{Society} ($\alpha = 4.0$): Heavily penalizes
...
stakeholder exceeds $\costhresh$ in every case. The differentiation
...
the learned weight vector? For the structural family
...
recovered $\hat\alpha$ values preserve perfect monotonic ordering:
...
We stress-test $\alpha$-recovery across four dimensions (1{,}300
...
are rarely dominated in 3D (0\% for society)---the cost of partial
...
six methods, focusing on hiding society (the highest-stakes case):
...
The \emph{oracle} feature---standard deviation of the residual from
...
\begin{itemize}
  ...
  \item \textbf{More data} (fixed $\sigma = 0.3$): increase $N$ from
    25 to 2{,}000. Hausdorff \emph{increases} after $N = 100$---from
...
Synthesizing across all Direction~2 experiments:
