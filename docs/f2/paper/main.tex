...
condition governing when this occurs: in Bradley-Terry (BT) preference
...
under softmax-weighted evaluation and 91\% (29/32) under hard
...
This paper formalizes this insight as the \emph{directional Goodhart
...
literature. Skalse et al.~\cite{skalse2024goodhart} distinguish four mechanisms:
...
Empirically, the condition holds at 100\% accuracy ($|\cos| > 0.2$,
...
all 3 top-$K$ violations. Both are consequences of its sparse,
...
stakeholders with sparse, high-variance weight structures (3 of~32
strong-cosine pairs on MIND; see \cref{sec:selection_mechanism}). The
...
all temperatures (except one seed-noise fluctuation on ML-100K at
$T = 0.5$).
