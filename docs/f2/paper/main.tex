...
% ══════════════════════════════════════════════════════════════════
\title{\textbf{Labels Not Loss: Multi-Stakeholder Differentiation,\\[2pt]
  Partial Observation, and Utility Sensitivity\\[2pt]
...
present the three research directions. \Cref{sec:nonlinear} tests
...
Direction~2 (\cref{sec:sensitivity}) tests the \emph{misweighting}
...
\centering
\begin{tikzpicture}[
  stage/.style={draw, rounded corners=4pt, minimum height=1.2cm, align=center, font=\small},
  data/.style={stage, fill=green!8, text width=2.8cm},
  proc/.style={stage, fill=blue!8, text width=2.8cm},
  eval/.style={stage, fill=orange!8, text width=2.8cm},
  metric/.style={stage, fill=red!8, text width=2.2cm, minimum height=0.9cm, font=\scriptsize},
  arrow/.style={-{Stealth[length=5pt]}, thick},
  note/.style={font=\scriptsize\itshape, text=gray!60!black},
]

% Row 1: Data generation
\node[data] (content) at (0, 0) {\textbf{Content Pool}\\[1pt]500 items\\18 actions each};
\node[data] (users) at (3.5, 0) {\textbf{Users}\\[1pt]600 users\\5 archetypes};
\node[proc] (utility) at (7, 0) {\textbf{Utility}\\$U_k = \text{pos} - \alpha_k \cdot \text{neg}$\\[1pt]$\alpha \in \{0.3, 1.0, 4.0\}$};
\node[proc] (pairs) at (10.5, 0) {\textbf{Preference Pairs}\\[1pt]2000 pairs\\per stakeholder};

\draw[arrow] (content) -- (users);
\draw[arrow] (users) -- (utility);
\draw[arrow] (utility) -- (pairs);

% Row 2: Training
\node[proc, fill=blue!12] (bt) at (3.5, -2.2) {\textbf{BT Training}\\[1pt]4 loss variants\\50 epochs};
\node[proc, fill=blue!15] (weights) at (7, -2.2) {\textbf{Weight Vectors}\\[1pt]$\mathbf{w} \in \mathbb{R}^{18}$\\per stakeholder};

\draw[arrow] (pairs) |- (bt);
\draw[arrow] (bt) -- (weights);

% Row 3: Evaluation
\node[eval, text width=3.4cm] (scorer) at (0, -2.2) {\textbf{Content Scorer}\\$s = \mathbf{w}^\top \mathbf{p}$\\+ diversity knob $\delta$};
\node[eval, text width=3.4cm] (frontier) at (0, -4.2) {\textbf{Pareto Frontier}\\[1pt]21 diversity weights\\3 utility dims};

\draw[arrow] (weights) -- (scorer);
\draw[arrow] (scorer) -- (frontier);

% Metrics
\node[metric] (regret) at (3.5, -4.2) {Regret\\(LOSO)};
\node[metric] (recovery) at (5.7, -4.2) {Recovery\\rate};
\node[metric] (rankstab) at (7.9, -4.2) {Rank\\stability};
\node[metric] (hausdorff) at (10.1, -4.2) {Hausdorff\\distance};
...
% Direction labels
\node[note, text width=3cm, align=right] at (13, 0) {Direction 1:\\What does BT\\recover?};
\node[note, text width=3cm, align=right] at (13, -2.2) {Direction 3:\\What's lost\\when hidden?};
\node[note, text width=3cm, align=right] at (13, -4.2) {Direction 2:\\How precise\\must spec be?};

\draw[dotted, gray] (11.5, 0.8) -- (11.5, -5);

\end{tikzpicture}
\caption{System pipeline. Content and users produce preference pairs
  via stakeholder utility functions. BT training yields 18-dim weight
  vectors. Scoring and Pareto frontier computation enable four
  evaluation metrics, each addressing a different research direction.}
\label{fig:pipeline}
\end{figure*}
...
%═══════════════════════════════════════════════════════════════════
\section{Direction 1: Identifiability}
...
%═══════════════════════════════════════════════════════════════════
\section{Direction 3: Partial Observation}
\label{sec:partial}
%═══════════════════════════════════════════════════════════════════
...
\begin{table}[t]
  ...
  \begin{tabular}{lcccc}
    ...
    \midrule
    Society  & $\mathbf{1.082 \pm 0.041}$ & $1.100 \pm 0.046$ & 0.0\% & 4.0 \\
    Platform & $0.369 \pm 0.062$ & $0.308 \pm 0.014$ & 0--8.5\% & 0.3 \\
    User     & $0.111 \pm 0.013$ & $0.141 \pm 0.025$ & 6.4--14.4\% & 1.0 \\
    \bottomrule
  \end{tabular}
\end{table}
...
0.644---a 42\% improvement (\cref{fig:exp4}). The marginal value of additional pairs
...
at $K=5$, 0.182 at $K=10$)---a modest 10\% improvement at $K=5$
...
%═══════════════════════════════════════════════════════════════════
\section{Direction 2: Utility Sensitivity}
...
(Direction~3), but \emph{amplifies errors} when specification is
...
Synthesizing across all Direction~2 experiments (\cref{tab:sensitivity}):
...
\textbf{Exp~A (Labels vs.\ Loss):} All mean cosine similarities
exceed 0.92 across families---the labels-not-loss claim generalizes
to nonlinear utilities.

\textbf{Exp~B ($\alpha$-Recovery):} Both families preserve
...
\textbf{Exp~C (Proxy Recovery):} The diversity knob is invariant
...
\textbf{Exp~D (Stress $\times$ Nonlinearity):} The same 4D stress
...
0.4112, a 59\% improvement over the untrained baseline (0.2585). An
...
\textbf{Direction~1 (Identifiability)} establishes what BT training
...
\textbf{Direction~3 (Partial Observation)} quantifies what is
...
\textbf{Direction~2 (Sensitivity)} reveals how \emph{precisely} the
...
\textbf{Three stakeholders.} Charter~E tests $K > 3$ but with
...
\begin{table}[H]
\caption{Exp~A: Labels vs.\ Loss under nonlinear utilities (mean cosine similarity across 4 loss types).}
\centering\small
\begin{tabular}{lccc}
\toprule
...
\begin{table}[H]
\caption{Exp~B: $\alpha$-recovery under nonlinear utilities.}
\centering\small
\begin{tabular}{lccc}
\toprule
Family & $\alpha$-Spearman & Sensitivity & Bias \\
\midrule
Concave ($\gamma = 0.7$) & 1.0000 & 0.169 & +57\% inflation \\
Threshold ($\tau = 0.1$) & 1.0000 & 0.154 & $-$40\% deflation \\
\bottomrule
\end{tabular}
\end{table}

\begin{table}[H]
\caption{Exp~C: Proxy recovery under nonlinear utilities (hiding society, 500-item pool).}
\centering\small
\begin{tabular}{lcccc}
\toprule
...
\section{Stress $\times$ Nonlinearity (Exp D)}
\label{app:stress}

\begin{table}[H]
\caption{Exp~D: Breaking points (Spearman~$< 0.95$) under nonlinear stress.}
\centering\small
\begin{tabular}{lccc}
\toprule
...
\begin{table}[H]
\caption{Charter~E: Scaling with $K$ stakeholders (F=8 factors, 101 diversity weights, 10 configs $\times$ 5 seeds).}
\centering\small
\begin{tabular}{lcccc}
\toprule
