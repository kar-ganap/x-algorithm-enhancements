...
% 2023: Explicit weights
\node[box, fill=blue!8] (scorer2023) at (0, 0) {\textbf{Phoenix Heavy Ranker}\\[2pt]MaskNet neural network};
\node[weight] (w1) at (-2.2, -1.8) {fav: 1.0};
\node[weight] (w2) at (-0.7, -1.8) {repost: 0.8};
\node[weight] (w3) at (0.8, -1.8) {block: $-$1.5};
\node[weight] (w4) at (2.3, -1.8) {report: $-$3.0};
...
% Arrow between eras
\draw[-{Stealth[length=8pt]}, ultra thick, gray!50] (4.5, 0) -- node[above, font=\small\itshape] {``eliminated all} node[below, font=\small\itshape] {hand-engineered features''} (7.5, 0);

% 2026: Implicit weights
\node[box, fill=red!8] (scorer2026) at (11, 0) {\textbf{Grok Transformer}\\[2pt]8B learned parameters};
\node[opaque] (t1) at (8.8, -1.8) {P(fav)};
\node[opaque] (t2) at (10.0, -1.8) {P(repost)};
\node[opaque] (t3) at (11.2, -1.8) {P(block)};
\node[opaque] (t4) at (12.5, -1.8) {P(report)};
\node[font=\scriptsize, text=gray] at (13.5, -1.8) {$\cdots$};
...
\node[label] at (11, -2.7) {Action \emph{types} visible, weights \textbf{opaque}};
\node[font=\small\bfseries] at (11, 1.2) {January 2026};
...
% Row 3: Evaluation
\node[eval] (scorer) at (0, -2.2) {\textbf{Content Scorer}\\$s = \mathbf{w}^\top \mathbf{p}$\\+ diversity knob $\delta$};
\node[eval] (frontier) at (0, -4.2) {\textbf{Pareto Frontier}\\[1pt]21 diversity weights\\3 utility dimensions};
