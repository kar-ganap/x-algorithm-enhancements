...
Across 13 (dataset, hidden stakeholder) configurations
(\cref{fig:budget}), the \textbf{median $N_{50\%}$ is 33~pairs}
and the \textbf{median $N_{80\%}$ is 104~pairs}. The practical cost is modest: 33
...
the cosine-sign prediction. An additional 30 MovieLens-specific
Method~B data points (5 named stakeholders $\times$ 3 targets
$\times$ 2 datasets) are available in the repository but are not
included in the cross-dataset pooled count.

\section{Data Budget Full Table}
\label{app:data_budget}

For each of the 13 (dataset, hidden stakeholder) configurations, we
...
\begin{table}[h]
  \centering\small
  \caption{Match rate at $|\text{trained cos}| > 0.2$ by selection
    temperature. Hard top-10 is shown for comparison.}
  \label{tab:temp_sweep}
  \begin{tabular}{lrrrr}
    \toprule
    \textbf{Selection} & \textbf{ML-100K} & \textbf{MIND} &
      \textbf{Amazon} & \textbf{Pooled} \\
    \midrule
    Hard top-10 & 6/6 & 9/14 & 8/8 & 23/28 \\
    $T = 0.1$   & 6/6 & 11/14 & 8/8 & 25/28 \\
    $T = 0.5$   & 5/6 & 11/14 & 8/8 & 24/28 \\
    $T = 1.0$   & 6/6 & 14/14 & 8/8 & 28/28 \\
    $T = 2.0$   & 6/6 & 14/14 & 8/8 & 28/28 \\
    $T = 10.0$  & 6/6 & 14/14 & 8/8 & 28/28 \\
    \bottomrule
  \end{tabular}
\end{table}
...
weights (same stakeholder, same seed). Across all 13~stakeholders and
3~datasets, the within-stakeholder cosine between standard BT and each
variant exceeds~0.93, confirming that the finding is robust to
