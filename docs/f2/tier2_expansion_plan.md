...
**Status**: All phases (A–F) complete. Tier 2 expansion is done.
...
| **D** | Run all experiments on Amazon Kindle | ~3 hours actual | **Complete 2026-04-11, 1 Phase B miss caught pre-flight** |
| **E** | Cross-dataset analysis + paper rewrite | ~2 days actual | **Complete 2026-04-17, selection mechanism finding + full paper** |
| **F** | Tests + docs | ~1 hour actual | **Complete 2026-04-17, 35 new tests, 327 total passing** |
...
- **2026-04-11**: Phase D complete. All 8 scripts ran on amazon-kindle (17K items, 32 features, 5 stakeholders). 1 additional Phase B miss caught pre-flight during Phase D planning: `run_extremal_evidence.py` line 325 hardcoded `"target": "user"` in selection_concentration config block (same K-dependent pattern as Phase C misses; 4th total). Fixed in Commit 1 with ml-100k + mind-small byte-equivalence re-verification. Zero Phase B misses surfaced during Phase D execution — the pre-flight grep of output-JSON string literals caught the last remaining bug. Scientific findings: (1) The 3 orthogonal-by-construction pairs (premium_seller vs publisher/indie_author/diversity) stay near zero under BT training, max |trained| = 0.048 — BT does not manufacture cosine from nothing. This is the cleanest probe of the direction condition's cos=0 boundary across all 4 datasets. (2) 1 meaningful sign flip (reader-publisher raw +0.311 → trained -0.076), replicating the MIND pattern at lower magnitude. (3) Mean |raw-trained| delta = 0.153 on Amazon (comparable to MIND, much larger than MovieLens) — the raw-vs-trained cosine ambiguity is a structural feature of 5-stakeholder/30+ dim datasets, not a MIND artifact. (4) data_budget recovery at N=25 ranges from 2.0% (`hide_publisher`) to 91.8% (`hide_reader`) — the paper's §6 "N=25 recovers most harm" claim needs a range statement or worst-case caveat. (5) Composition projection cosine 0.877 on Amazon (vs MIND 0.893, ml-100k ~0.98) — §5 low-rank claim should be framed as a spectrum parameterized by basis overlap, not a binary. (6) All 5 Amazon stakeholders cleared labels_not_loss within-loss > 0.85 threshold (weakest: indie_author 0.890) — unlike MIND where 2 of 5 failed; the noisy-balance → labels-not-loss trouble heuristic doesn't generalize. Full retro at `docs/f2/tier2_phase_d_retro.md`. Phase E (cross-dataset analysis + figures) greenlit.
- **2026-04-17**: Phase E complete. Paper fully written — 17 pages, 9 sections + 6 appendix sections, no remaining placeholders. Key Phase E contributions: (1) Convergence ablation falsified the within-loss > 0.85 threshold hypothesis (Spearman ρ = -0.006, p = 0.98). (2) Selection mechanism ablation discovered the paper's central methodological finding: the direction condition holds at 32/32 (100%) for |cos|>0.2 under softmax T≥1.0 across 4 datasets, but drops to 29/32 (91%) under hard top-K — all 3 violations on MIND journalist, caused by high utility variance near the selection boundary, not cosine geometry. Transition at T=0.5→1.0. (3) Full-pool vs top-K diagnostic confirmed the mechanism: journalist's full-pool utility follows the direction condition perfectly; only top-K reverses it. (4) Proposition 4.1 rewritten as a theorem about expected utility under the score distribution, with trained cosines (not raw). (5) §3 expanded with MIND/Amazon dataset descriptions. (6) §5 written: labels-not-loss + composition as spectrum (ml-100k 0.979, MIND 0.893, Amazon 0.877). (7) §6 rewritten with N-for-target framing (median N_50%=33, N_80%=104). (8) Fig 3 regenerated with 52 points across 4 datasets, trained cosines on x-axis. (9) Intro rewritten to distinguish multi-stakeholder Goodhart as qualitatively new (ex-ante predictability from cosine geometry, not reducible to single-stakeholder proxy optimization). (10) Appendix A–F written with self-contained content (no codebase file paths). Full retro at `docs/f2/tier2_phase_e_retro.md`. Phase F (tests + docs) greenlit.
- **2026-04-17**: Phase F complete. 35 new tests across 3 files: `test_mind_stakeholders.py` (14 tests: weight properties, content pool, preferences, disagreement, BT convergence), `test_amazon_stakeholders.py` (14 tests: same structure + orthogonal pair verification), `test_expanded_validation.py` updated with `TestMethodAMultiDataset` (7 tests: parametrized cosine matrix validation across ml-100k, mind-small, amazon-kindle via dataset registry). All 327 tests pass. 3 lint violations in Phase E's `run_convergence_ablation.py` fixed opportunistically. Separate loader tests (`test_mind_loader.py`, `test_amazon_loader.py`) skipped — stakeholder test fixtures already exercise the loaders. Full retro at `docs/f2/tier2_phase_f_retro.md`. **Tier 2 expansion complete.**
